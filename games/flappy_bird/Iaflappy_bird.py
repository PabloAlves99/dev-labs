...
def main(genomas, config):  # fitness function
    ...
    while running:
        ...
        pipe_index = 0
        if len(birds) > 0:
            if len(pipe) > 1 and birds[0].bird_x > \
                    (pipe[0].pipe_x + pipe[0].pipe_top.get_width()):
                pipe_index = 1
        else:
            running = False
            break
        ...
        # mover as coisas
        for i, _bird in enumerate(birds):
            _bird.bird_move()
            # aumentar um pouquinho a fitness do passaro
            list_genomas[i].fitness += 0.1
            output = networks[i].activate((_bird.bird_y,
                                           abs(_bird.bird_y -
                                               pipe[pipe_index].pipe_height),
                                           abs(_bird.bird_y -
                                               pipe[pipe_index].
                                               pipe_base_position)))
            ...
            for i, bird in enumerate(birds):
                if _pipe.colide(bird):
                    ...
                    if ia_playing:
                        ...
                        list_genomas.pop(i)
                        networks.pop(i)
                if not _pipe.pipe_surpassed and _bird.bird_x > _pipe.pipe_x:
...
if __name__ == '__main__':
    ...
    _path_config = os.path.join(_path, 'iaconfig.txt')
    rotate(_path_config)
